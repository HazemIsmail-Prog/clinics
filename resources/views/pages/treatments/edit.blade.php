    ...
    <form action="{{route('treatments.update',$treatment->id)}}" method="post">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Edit Treament - {{$treatment->name}}</h6>
            </div>
            <!-- Card Body -->
            <div class="card-body">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control {{$errors->has('name')?'border-danger':''}}" type="text"
                                   id="name"
                                   name="name" value="{{old('name',$treatment->name)}}">
                            @if($errors->has('name'))
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control {{$errors->has('price')?'border-danger':''}}" type="number"
                                   step="0.001"
                                   id="price"
                                   name="price" value="{{old('price',$treatment->price)}}">
                            @if($errors->has('price'))
                        ...
                        <div class="form-group">
                            ...
                            <select
                                ...
                                @foreach($departments as $department)
                                    <option
                                        value="{{$department->id}}" {{old('department_id',$treatment->department_id) == $department->id ? 'selected' : ''}}>{{$department->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-check">
                                <input class="form-check-input"
                                       {{old('active',$treatment->active) == 1 ? 'checked' : ''}} name="active"
