...
@section('content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">Journal Vouchers</h6>
            @can('JournalVouchers_Create')
                <div class="dropdown no-arrow">
                    <a class="btn btn-sm btn-outline-primary" href="{{route('jvs.create')}}">New Journal Voucher</a>
                </div>
            @endcan
        </div>
        <!-- Card Body -->
        <div class="card-body">
            ...
            <div class="row">
                <form action="{{route('jvs.index')}}" id="search_form" class="my-2 form-inline w-100" method="get">
                    @method("GET")
                    ...
                    <div class="col-md-6">
                        ...
                        <a href="{{route('jvs.index')}}" class="btn btn-primary btn-sm">Clear Filter</a>
                    </div>
                                    ...
                                    <td rowspan="{{$voucher->voucher_details->count()}}" class="text-center">
                                        ...
                                        <div class="mt-1">
                                            ...
                                            @can('JournalVouchers_Update')
                                                ...
                                                @endif
                                            @endcan
                                            @can('JournalVouchers_Delete')
                                                @if(date('Y', strtotime($voucher->voucher_date)) > auth()->user()->clinic->account_group->last_closed_year)
                                                    <a
                                                        class="btn btn-outline-danger btn-sm"
                                                        href="{{route('jvs.destroy',$voucher->id)}}"
                                                    ...
                                                    <form
                                                        action="{{route('jvs.destroy',$voucher->id)}}"
