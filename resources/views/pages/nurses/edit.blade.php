    ...
    <form action="{{route('nurses.update',$nurse->id)}}" method="post">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Edit Nurse - {{$nurse->name}}</h6>
            </div>
            <!-- Card Body -->
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{$errors->has('name')?'border-danger':''}}" type="text"
                                   id="name"
                                   name="name" value="{{old('name',$nurse->name)}}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <select
                                ...
                                @foreach($departments as $department)
                                    <option
                                        value="{{$department->id}}" {{old('department_id',$nurse->department_id) == $department->id ? 'selected' : ''}}>{{$department->name}}</option>
                                @endforeach
                            </select>
