    ...
    <div class="container">
            ...
            <table>
                <thead>
                    ...
                    <tr>
                        <td colspan="9" class="page-title">User\Doctor Wise Detailed View</td>
                    </tr>
                    <tr>
                        <td colspan="9" class="page-info">Report From {{date('d-m-Y',strtotime($start_date))}}
                ...
                <tr>
                    <td colspan="9" class="account_name">
                        {{$row->name}}
                    </td>
                </tr>
                ...
                @foreach($row->invoices->groupBy('doctor_id') as $key => $invoices)
                    <tr>
                        <td colspan="9" style="font-weight: bold">{{\App\Models\Doctor::find($key)->name }}</td>
                    </tr>

                    @foreach($invoices as $invoice)
                        <tr>
                            <td nowrap>{{date('d-m-Y H:i', strtotime( $invoice->created_at))}}</td>
                    ...
                    @endforeach
                    <tr>
                        <th style="font-weight: bold" colspan="2">{{\App\Models\Doctor::find($key)->name }}'s Total</th>
                ...
                @endforeach
                <tr>
                    <th colspan="2">{{$row->name}}'s Total</th>
